...
!function (exports, complex_array) {
	var ComplexArray = complex_array.ComplexArray,
		...
		PI = Math.PI, TAU = 2 * PI, SQRT1_2 = Math.SQRT1_2, sqrt = Math.sqrt, cos = Math.cos, sin = Math.sin;
	...
	function FFT_Recursive(input, inverse) {
		var n = input.length,
			...
			p, m, normalisation, recursive_result, _swap, _real, _imag,
			sens = (inverse ? -1 : 1), del_f_b;
		if (n === 1) {
			return input;
		}
	...
	function FFT_2_Iterative(input, inverse) {
		var n = input.length,
			// Counters.
			i, j, output, output_r, output_i,
			// Complex multiplier and its delta.
			f_r, f_i, del_f_r, del_f_i, temp,
			// Temporary loop variables.
			l_index, r_index, left_r, left_i, right_r, right_i,
			// width of each sub-array for which we're iteratively calculating FFT.
			width,
			sens = (inverse ? -1 : 1), del_f_b, l, l_index_b;
		...
		while (width < n) {
			...
			del_f_r = cos(del_f_b);
			del_f_i = sin(del_f_b) * sens;
			for (i = 0, l = n / (2 * width); i < l; i++) {
				f_r = 1;
				f_i = 0;
				for (j = 0, l_index_b = 2 * i * width; j < width; j++) {
					l_index = l_index_b + j;
